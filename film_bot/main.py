...
bot = Bot(token="6059626422:AAE2a4blK5j-KXb3M8ijrzbZYh9PMTqLC14")
...
async def send_welcome(message):
    ...
    if len(channels_me) == 0:
        ...
        for i in channels_me:
            ...
            if user_channel_status["status"] == 'left':
                ...
                await bot.send_message(chat_id = message.from_user.id, text = '*📝 Для використання бота, ви повинні бути підписані на наші канали:*',reply_markup = inline_kb1,parse_mode = 'Markdown')
                c = 0
                break
            else:
                c = 1
    if c == 1:
       ...
       await bot.send_message(message.from_user.id, text = '*привіт це Український бот фільм | Пошук🔍\nЩоб дізнатися про назву фільмів з ЮТУБ | ТІК ТОК вам потрібно ввести код фільму.\nНатисніть на «🔎Пошук» 👇*',parse_mode = 'Markdown',reply_markup = start.greet_kb1)
...
async def vote_up_cb_handler(call):
    ...
    await bot.send_message(call.from_user.id, text = '*📈Кількість людей у боті: ' + str(len(q)) + '*',parse_mode = 'Markdown')
...
async def vote_up_cb_handler(call):
    ...
    await bot.send_message(call.from_user.id, text = '*Введіть номер, під яким фільм*',parse_mode = 'Markdown')
...
async def vote_up_cb_handler(call):
    ...
    if len(channels_me) == 0:
        ...
        for i in channels_me:
            ...
            if user_channel_status["status"] == 'left':
                ...
                inline_btn_2 = InlineKeyboardButton('Поиск', callback_data='check')
                inline_kb1.add(inline_btn_2)
                await bot.send_message(chat_id = call.from_user.id, text = '*📝 Для використання бота, ви повинні бути підписані на наші канали:*',reply_markup = inline_kb1,parse_mode = 'Markdown')
                c = 0
                break
            else:
                c = 1
    if c == 1:
       ...
       await bot.send_message(call.from_user.id, text = '*Привіт це Український бот  | Пошук🔍\nЩоб дізнатися назву фільмів із ЮТУБ | ТІК СТРУМ вам потрібно ввести код фільму.\nНажмите на «🔎Поиск» 👇*',parse_mode = 'Markdown',reply_markup = start.greet_kb1)
...
async def send_welcome(message):
    ...
    if c == 1:
        ...
        if str(message.text) == '🔎Поиск':
            a = 1
            await bot.send_message(chat_id = message.from_user.id,text = '*🔎 Для пошуку надішліть КОД фільму/серіалу*',parse_mode = 'Markdown')
        ...
        if a == 0:
            ...
            if str(message.text) in q:
                try:
                    await bot.send_photo(chat_id = message.from_user.id, photo = str(q[str(message.text)][0]),caption = '*🎬 Назва фільму "' + q[str(message.text)][1] + '"*',parse_mode = 'Markdown')
                except Exception as ex:
                    print(ex)
